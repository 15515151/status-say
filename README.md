...
脱敏在服务端完成：仅输出解析次数、参与用户数量、已知平台名称、日期及媒体聚合值；群号、群名、QQ 号、OpenID、昵称、头像、群排行、用户排行、模板路径、资源路径与图片渲染字段全部丢弃。平台名使用固定白名单，未知平台合并为「其他」，时长与大小仅接受限定格式，嵌套对象不透传。本站不提供按群查询或上游统计图片代理，`group_id`、`format`、URL 等查询参数均拒绝。
...
浏览器仅请求同源 `/api/status?hours=24`、`/api/accounts`、`/api/group-stats`、`/api/parse-stats` 及账号头像代理。服务端读取 `.env` 并为 AI / 锅巴接口附加各自的鉴权信息；群组统计、解析统计和头像请求不携带这些凭据，凭据不会写入前端源码、Vite 环境变量或构建产物。
